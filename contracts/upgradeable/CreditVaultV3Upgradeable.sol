...
import "./ScoreOraclePhase3BUpgradeable.sol";
import "../ChainlinkPriceOracle.sol";

/**
 * @title CreditVaultV3Upgradeable
 * @notice UUPS Upgradeable lending vault with credit scoring integration
 * @dev Phase 3 - Production-ready with upgrade capability
 *
 * UPGRADE PATTERN: UUPS (Universal Upgradeable Proxy Standard)
 * - Allows updating lending logic without data migration
 * - Owner-controlled upgrades via _authorizeUpgrade()
 * - Loan data preserved across upgrades
 *
 * FEATURES:
 * - Credit score-based LTV and APR
 * - Chainlink price feeds for collateral valuation
 * - Grace periods based on tier
 * - Emergency pause mechanism
 * - Insurance pool for bad debt
 */
    ...
    UUPSUpgradeable
{
    ...
    ScoreOraclePhase3BUpgradeable public oracle;
    ...
    function borrow(
        ...
        ScoreOraclePhase3BUpgradeable.ScoreBreakdown memory score = oracle.computeScore(msg.sender);
    ...
    function repay(uint256 loanId, uint256 amountUsd18) external nonReentrant whenNotPaused {
        ...
        CreditRegistryV3Upgradeable.LoanRecord memory loan = registry.getLoan(loanId);
        require(loan.status == CreditRegistryV3Upgradeable.LoanStatus.Active, "Loan not active");
    ...
    function _checkLiquidationEligibility(
        ...
        ScoreOraclePhase3BUpgradeable.ScoreBreakdown memory score = oracle.computeScore(borrower);
        uint16 maxLtvPercent = _maxLtvForScore(score.overall);
    ...
    function _calculateDebt(uint256 loanId) internal view returns (uint256) {
        ...
        if (loan.status != CreditRegistryV3Upgradeable.LoanStatus.Active) {
            return 0;
        }
